        ...
        DispatchQueue.global(qos: .background).async {
                Timer.scheduledTimer(withTimeInterval: 2, repeats: true) { _ in
                    ...
                    if let speed = speed {
                        ...
                        print("Listener Calls               \(self.numberFormatter.string(from: speedListener)!) per/sec")

                        DispatchQueue.main.async {
                            self.eventsCounterLabel.text = "Event speed: \(self.numberFormatter.string(from: speed)!)"
                        }
                    }
                }
                RunLoop.current.run()
